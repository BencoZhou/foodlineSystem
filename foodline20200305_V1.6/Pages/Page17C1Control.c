...
//    u16 tempSelect, tempState, tempButton, tempAlarm;
    ...
    if(addr == PAGE17_START_BUTTON)
    {
        ...
        if(DeviceControlParaGet()->stateMachineState[DEVICE_AREA_C-1] == STATE_CHANGE_SUSPEND)
        {
...
void Page17C1ControlRefresh(void)
{
    ...
    {
		if(AllTheControlParaGet((DEVICE_AREA_C - 1),i)->cDevice.place.type == DEVICE_NAME_TOWERS_OUT)
		{
			AllTheControlParaGet((DEVICE_AREA_C - 1),i)->rotationDirection = TOWERSOUT_CONTROL_FOREWARD;   //控制绞龙正转
		}		
		if(AllTheControlParaGet((DEVICE_AREA_C - 1),i)->cDevice.place.mainLine != 0)
		{
			...
			DisplayCommIconSend(PAGE17_STATE_MACHINE_STATE     , DeviceControlParaGet()->stateMachineState[DEVICE_AREA_C-1]);
