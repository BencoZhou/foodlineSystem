...
//    u16 tempSelect, tempState, tempButton, tempAlarm;
    ...
    if(addr == PAGE19_START_BUTTON)
    {
        ...
        if(DeviceControlParaGet()->stateMachineState[DEVICE_AREA_N-1] == STATE_CHANGE_SUSPEND)
        {
...
void Page19N1ControlRefresh(void)
{
    ...
    for(i = 1; i < SING_LINK_DEVICE_TOTAL_NUMBER; i++)
    {
		...
		{
			AllTheControlParaGet((DEVICE_AREA_N - 1),i)->rotationDirection = TOWERSOUT_CONTROL_FOREWARD;   //控制绞龙正转
		}			
        ...
        DisplayCommIconSend(PAGE19_STATE_MACHINE_STATE     , DeviceControlParaGet()->stateMachineState[DEVICE_AREA_N-1]);
        OSTimeDly(2);
