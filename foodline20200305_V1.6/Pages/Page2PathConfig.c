...
void Page2PathConfigInsert(u8 index)
{
	u8 i, num;
    num = PAGE2_DEVICE_PER*PAGE2_DEVICE_PAGES;
	if(index >= num)
		return;
	for(i = num-1; i > index; i--)
	{
        PathParameterGet()->dPara[gIndex][i].id = PathParameterGet()->dPara[gIndex][i-1].id;
	}
    PathParameterGet()->dPara[gIndex][index].placeNew.useID = 0;
}

void Page2PathConfigDelete(u8 index)
{
	u8 i, num;
    num = PAGE2_DEVICE_PER*PAGE2_DEVICE_PAGES;
	if(index >= num)
    ...
    PathParameterGet()->dPara[gIndex][num-1].placeNew.useID = 0;
}
...
void Page2PathConfigProcess(u8 reg, u16 addr, u8 *pbuf, u8 len)
{
    ...
    if(tempIndex < PAGE2_DEVICE_PER && gP2Table < PAGE2_DEVICE_PAGES)
    {
        ...
        if(tempAddr == PAGE2_PATH_INSERT1)
        {
		    Page2PathConfigInsert(tempIndex+gP2Table*PAGE2_DEVICE_PER);
        }
        if(tempAddr == PAGE2_PATH_DELETE1)
        {
		    Page2PathConfigDelete(tempIndex+gP2Table*PAGE2_DEVICE_PER);
        }
